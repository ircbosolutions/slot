...
const Orqs = ({ expiryTimestamp }) => {
    ...
    const getOrqsData=()=>{
        ...
        Axios.get('http://localhost:3002/getorqsquesdata/'+courseid+"/"+attempt).then((response)=>{
   ...
   const userCourseid= ()=>{
    ...
    const orqsData=()=>{
        ...
        const attempt = localStorage.getItem("user_attempt")
        Axios.put('http://localhost:3002/sendusrorqsdata',{
            courseID:courseID,
            uid:getUid,
            q1:q1,
            ans1:ans1,
    ...
    // );
      ...
      } = useTimer({ expiryTimestamp, onExpire: () => orqsData() });
...
<br />
                <div className="orqs-container-full">
                    ...
                    <br />
                    <div className='textarea_style'>
                    <textarea id="non_paste3" maxLength='500' minLength='20' name="" cols="100" placeholder='Enter Your Answer Here' rows="5" onChange={sendAns3}/>
                    ...
                    <div >
                    <Link className='orqs-submit' onClick={orqsData}>Submit</Link>
                    </div>
